...
public sealed class CleanupService(Discovery discovery)
{
    ...
    public async Task<ExitResult> CloseAsync(AppSnapshot app,IProgress<(string AppId,string Message)>? progress,CancellationToken token,int observeMs=5000)
    {
        ...
        var response=await Task.Run(()=>RestartManager.Request(roots,request.Token),CancellationToken.None);
        ...
        if(app.Definition.Executables.Any(n=>discovery.InaccessibleNames.GetValueOrDefault(n)>0))return Result(ExitStatus.AccessUnavailable,"A current app process could not be identified; exit cannot be confirmed.",response.Code,current?.Ram??0);
        ...
        return Result(response.Code==5?ExitStatus.AccessUnavailable:ExitStatus.StillRunning,response.Code==351?"App declined exit or needs attention.":response.Code==1223?"Exit canceled; app is still running.":"Still running; no forced shutdown used.",response.Code,current?.Ram??0);
    }
}
